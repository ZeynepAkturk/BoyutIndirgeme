function [bestSolution, bestFitness]=case_3(veri_seti, test_verisi,komsu_sayisi )

[nPop, dimension, maxIteration, lbArray, ubArray] = problem_terminate(veri_seti);
...
%Calculating the fitness of first created points=========================== 
    ...
    for i = 1 : size(point,1)
        FirstFit(i) = problem(point(i,:),veri_seti,test_verisi, komsu_sayisi); 
    end
...
while ( ( nfeval < S.Maximum_Generation) )
    ...
    for i = 1 : S.Start_Point
        Fit_FirstProcess = problem(P(i,:),veri_seti,test_verisi, komsu_sayisi); 
    ...
    for i = 1 : S.Start_Point
       if rand > Pa(i)
           ...
           R2 = ceil(rand*size(point,1));
            ...
            replaceFit =  problem(ReplacePoint,veri_seti,test_verisi, komsu_sayisi);  
...
bestSolution=pbest;
    ...
    fprintf('En iyi uygunluk değeri:%d\n'); disp(bestFitness);
    fprintf('En iyi çözüm adayı:'), disp(bestSolution); 
    fprintf("Minimum değer:"); disp(min(Sorted_FitVector));
    fprintf("Maximum değer:"); disp(max(Sorted_FitVector));
    fprintf("Ortanca değer:"); disp(median(Sorted_FitVector));
    fprintf("Ortalama değer:"); disp(mean(Sorted_FitVector));
    fprintf("Standart Sapma değeri:"); disp(std(Sorted_FitVector));
...
function [createPoint, fitness] = Diffusion_Process(Point,Fitness,S,g,BestPoint, fhd, fNumber)
    ...
    for i = 2 : size(New_Point,1)
        fitness(i) = problem(New_Point(i,:),veri_seti,test_verisi, komsu_sayisi); 
    end
