function [BestSolution, BestFitness]=case_1(egitim_veri, test_veri,komsu_sayisi )

[nPop, dimension, maxIteration, lbArray, ubArray] = problem_terminate(egitim_veri);
...
%Calculating the fitness of first created points=========================== 
    ...
    for i = 1 : size(point,1)
        FirstFit(i) = problem(point(i,:),egitim_veri,test_veri, komsu_sayisi); 
    end
...
while ( ( nfeval < S.Maximum_Generation) )
    ...
    for i = 1 : S.Start_Point
        Fit_FirstProcess = problem(P(i,:)',egitim_veri,test_veri,komsu_sayisi); 
    ...
    for i = 1 : S.Start_Point
       if rand > Pa(i)
           ...
           R2 = ceil(rand*size(point,1));
            ...
            replaceFit =  problem(ReplacePoint,egitim_veri,test_veri,komsu_sayisi); 
...
BestSolution=pbest;
    ...
    fprintf('BestFitness'); disp(BestFitness);
    fprintf('BestSolution:'), disp(BestSolution); 
...
function [createPoint, fitness] = Diffusion_Process(Point,Fitness,S,g,BestPoint, fhd, fNumber)
    ...
    for i = 2 : size(New_Point,1)
        fitness(i) = problem(New_Point(i,:),egitim_veri,test_veri,komsu_sayisi); 
    end
